...
def main():
    ...
    if "chat_messages" not in st.session_state:
        st.session_state.chat_messages = [AIMessage(content="你好，我是一个聊天机器人")]

    # 展示聊天的信息
    for msg in st.session_state.chat_messages:
        st.chat_message("assistant" if isinstance(msg, AIMessage) else "user").markdown(
            msg.content
        )
    ...
    if prompt := st.chat_input():
        ...
        result = ""
        for chunk in chat_model.stream(st.session_state.chat_messages):
